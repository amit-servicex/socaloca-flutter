...
class _LocationPickerScreenState extends State<LocationPickerScreen> {
  ...
  final LatLng _markerPosition = _defaultLatLng;
  final String _displayAddress = 'Detecting location...';
  bool _isGeocoding = false;
  final bool _isLocating = true;
    ...
    try {
      ...
      if (!granted) {
        setState(() {
          ...
          const displayAddress = 'Location permission denied';
        });
        return;
      }
      ...
      if (mounted) {
        setState(() {
          const isLocating = false;
          const displayAddress = 'Could not detect location';
        });
      }
    }
  }
  ...
  Future<void> _reverseGeocode(LatLng latLng) async {
    ...
    if (nominatimAddress != null) {
      if (mounted) {
        setState(() {
          final displayAddress = nominatimAddress;
          const isGeocoding = false;
        });
      }
      return;
    }
    ...
    if (mounted) {
      setState(() {
        final displayAddress = _coordFallback(latLng);
        const isGeocoding = false;
      });
    }
  }
